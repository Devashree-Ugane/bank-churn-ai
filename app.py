...
data_stats = load_json("models/data_stats.json")
metrics = load_json("models/metrics.json")
model_comparison = load_json("models/model_comparison.json")
retrain_history = load_json("models/retrain_history.json") or []
...
with tab2:
    ...
    if metrics:
        ...
        st.warning("No metrics found. Please run `train.py` first.")
    ...
    if model_comparison:
        ...
        st.info("Model comparison data not available. Run the updated `train.py` to generate it.")
...
with tab3:
    ...
    if st.button("🔄 Retrain Model Now"):
        with st.spinner("Retraining model... this may take a minute"):
            ...
            if result == 0:
                st.success("✅ Model retrained successfully! Reload the page to see updated metrics.")
            else:
                st.error("❌ Retraining failed. Check your terminal for details.")
    ...
    if retrain_history:
        ...
        st.info("No retraining history yet. Retrain the model to start tracking performance over time.")
    ...
    try:
        user_log = pd.read_csv("data/user_inputs.csv")
